  ...
  public  class Printcreditnote
    {
        ...
        private void PrintDocument_PrintPage(object sender, PrintPageEventArgs e)
        {
            ...
            top += 5;
            printInBox(left, top, width / 2, g, "Creditnote Id: " + Creditnote.id.ToString(), p.f_7, StringAlignment.Near);
            top += printInBox(160, top, 100, g, "Date: " + RetailControls.Helpers.DateStampToString(Creditnote.date.Value), p.f_7, StringAlignment.Near);
            top += 5;
            g.DrawLine(penBlack, left, top, left + width, top);
            top += 5;
            if (Creditnote.customer_id != null)
            {
                var customer = db.customers.Find(Creditnote.customer_id);
                if (customer != null)
                {
            ...
            top += RetailControls.Helper.drawtext(g,
                    ...
                    "Total"
                       ...
                       new double[]
                       {
                     0,0.1,0.3,0.2,0.1,0.1,0.2,
                       }, top, width - 10, p.f_7, brushBlack, 5, StringAlignment.Center
                        ) + 5;
            var bis = db.creditnoteitems.Where(o => o.creditnote_id == Creditnote.id).ToList();
            for (int i = 0; i < bis.Count; i++)
            {
                ...
                top += RetailControls.Helper.drawtext(g,
                    ...
                    (i+1).ToString(),
                    itemname,
                    item.qty.ToString("00"),
                    item.unitrate.ToString("00"),
                    item.unitdiscount.ToString("00"),
                    item.nettotal.ToString("00"),
                    },
                       new double[]
                       {
                     0,0.1,0.3,0.2,0.1,0.1,0.2,
